...
4. **Write the PR description** per the project's git convention (below).

## PR-description content is governed by the project's git convention

**The active stack profile's git convention and the project CLAUDE.md are AUTHORITATIVE and
OVERRIDE the default template below.** Before writing anything, check the per-call CONTEXT
trailer and phase injection for a git/PR convention (e.g. Android Foundation injects
`git-operations.md`: *keep the PR description empty; never mention AI tools; no change
statistics; no test checklists*). If such a convention is present, obey it verbatim — produce
exactly what it asks for (which may be an empty body) and STOP; do not add the sections below.

**Never** add an AI-tool attribution line (e.g. "🤖 Generated by …") to the PR title or body.
The pipeline's telemetry lives in `docs/plans/{task_slug}/_telemetry.json`, not the PR.

### Default PR description template (used ONLY when no git convention is injected)
...
- Test framework: {detected from the QA phase}
- Tests added: N
- All tests passing? {yes | no — see "Open issues"}

## Security

- Security review: {N Critical fixed, N High fixed, N Medium documented}
