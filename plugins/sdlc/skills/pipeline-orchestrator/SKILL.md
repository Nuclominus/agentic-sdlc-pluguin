...
Agent({
  ...
  run_in_background: false                   // MANDATORY — see below
})
```

🚨 **`run_in_background: false` is MANDATORY on every phase dispatch.** The pipeline is
strictly sequential: each phase reads the prior phase's file output
(`docs/plans/{task_slug}/0X-*.md`) before it can run. The `Agent` tool defaults to
**background** execution — if you omit `run_in_background: false`, the agent detaches, you
have no result to hand to the next phase, and you are forced to improvise a wait. Dispatch
every phase agent synchronously and block on its return before advancing. (Only the
`{parallel: [...]}` group in 3-parallel dispatches several agents in one message — but each is
still `run_in_background: false`; you await ALL of them before proceeding.)

**Do NOT use task-tracking tools** (`TaskCreate`, `TaskUpdate`, `TodoWrite`, or any
progress-tracker) to model pipeline progress. The "MUST PRINT VERBATIM" phase banners (3a-pre,
3b-2, 3-parallel, 3-loop) ARE the progress contract — they are the single source of progress
truth the user relies on. Reaching for a tracker tool here is unguided improvisation and has
produced `Invalid tool parameters` failures mid-run. Print the banners; do not track.
...
PR-description content is governed by the ACTIVE STACK PROFILE'S git conventions
(injected via phase_prompts_injection for this `documentation` phase) and the project's
CLAUDE.md — those are AUTHORITATIVE and OVERRIDE the defaults below. Read them first;
if a profile says "keep the PR description empty" or "never mention AI tools / no change
statistics / no test checklists", obey that verbatim and skip the corresponding sections.

Default PR description (used ONLY when the profile injects no git convention):
- Summary (1 paragraph)
...
Never add an AI-tool attribution line (e.g. "Generated by …") to the PR title or body
unless the project's git convention explicitly asks for one.
...
- Dispatch a phase agent in the background — always `run_in_background: false` (3c). The pipeline is sequential.
- Use `TaskCreate` / `TaskUpdate` / `TodoWrite` to track pipeline progress — the phase banners are the progress contract (3c).
