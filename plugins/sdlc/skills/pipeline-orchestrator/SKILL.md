...
| `extensions` | object with a `skills` array | Per-agent skill mapping injected into phase prompts in Step 3b-1. Parsed into `EFFECTIVE_PROFILE.extension_skills` (see 1b-ext). Additive — never replaces plugin behavior. |
...
- If rows match, render them deterministically — **mandatory first, then recommended; within each group
  sorted alphabetically by `skill`** (so the block is stable across runs and cache-friendly):
...
agent is deterministic, so its matched rows are identical across runs. Editing `sdlc.local.yaml`
legitimately invalidates the cache — that is the expected and only trigger. Do NOT splice any per-call
...
- The `project_extension_skills_block` (3b-1a) MUST be ordered deterministically (mandatory-first, then alphabetical by skill) and OMITTED entirely when no row targets the agent — never emit an empty header. It is invalidated only by edits to `sdlc.local.yaml`, which is acceptable.
