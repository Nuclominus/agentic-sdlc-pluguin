...
STOP and fix the vault if any item fails.

### PR creation (MANDATORY — read the git rule first)

Before running `gh pr create`, `Read` `${CLAUDE_PLUGIN_ROOT}/rules/git-operations.md` and obey it verbatim:
- **Keep the PR description EMPTY.** No change statistics, no test-plan checklists.
- **NEVER mention AI tools** (Claude, Copilot, Gemini, …) anywhere in the PR title or body — no "Generated by …" line.
- PR title format: `[task_id] Title` (e.g. `[CRF-6] Search Filters`).
