#!/usr/bin/env bash
# android-plugin SessionStart advisory: if this is an Android project and the optional
# Android CLI (`android`) is not installed, print a non-blocking note. Fails open.
# This keeps ALL Android-CLI logic inside android-plugin — the core orchestrator never sees it.
set -uo pipefail
...
cat <<'MSG'
[android-plugin] Optional: Android CLI (`android`) not found on PATH.
