...
| PR-3 | `git rm plugins/android-foundation/agents/`; schema forbids roster keys on non-core foundations; resolver ignores + warns; roster gains the `home` and `stragglers` checks; create-pluguin, CONTRIBUTING, README, marketplace, CHANGELOG, commands; android-foundation 2.0.0; ADR-0021 → accepted. | in review |
