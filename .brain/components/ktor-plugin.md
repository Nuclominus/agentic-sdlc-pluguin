...
into a `frameworks:` row (`stack: ktor`) inside `android-foundation/manifest.yaml`. Conditional
activation is unchanged (`enriches_aspect: network`); the skill id moved from
`ktor-plugin:*-conventions` to `android-foundation:*-conventions`. This note is kept, not deleted, so
