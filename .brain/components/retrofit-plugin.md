...
into a `frameworks:` row (`stack: retrofit`) inside `android-foundation/manifest.yaml`. Conditional
activation is unchanged (`enriches_aspect: network`); the skill id moved from
`retrofit-plugin:*-conventions` to `android-foundation:*-conventions`. This note is kept, not deleted, so
